...
const SortDataTable: React.FC<IDataTable> = ({
  ...
  const startItem = Math.max((nextPage - 2) * (itemPerPage ?? 10) + 1, 1) // eslint-disable-line no-mixed-operators
  const endItem = Math.min((nextPage - 1) * (itemPerPage ?? 10), totalItem) // eslint-disable-line no-mixed-operators
