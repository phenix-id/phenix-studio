...
function FormikData({
  ...
  const handleAttributeChange =
    ...
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      // Bug fix 1: preserve the user's casing — don't force lowercase
      formikHandlers.setFieldValue(
        `attribute.${index}.attributeName`,
        raw,
        true,
      )
      // Bug fix 2: only auto-fill displayName when the user hasn't manually
      // edited it. A display name is treated as "auto-generated" when it is
      // empty or still matches what toTitleCase would have produced from the
      // previous attributeName value.
      const prevAttributeName =
        formikHandlers.values.attribute[index].attributeName
      const currentDisplayName =
        formikHandlers.values.attribute[index].displayName
      const wasAutoGenerated =
        currentDisplayName === '' ||
        currentDisplayName === toTitleCase(prevAttributeName)
      if (wasAutoGenerated) {
        formikHandlers.setFieldValue(
